...
Public Class Admin___Category
    Dim CONN As New MySqlConnection("server = localhost;userid = root;database = hefflox")
    Dim command As MySqlCommand

    Private Sub Bin1()
        Dim command As New MySqlCommand("SELECT * FROM hefflox.product_table WHERE ProdID >= 1000 AND ProdID < 2000 ORDER BY ProdID ASC", CONN)
    ...
    Private Sub Axle_Hub__Categ__v2Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Bin1()
    End Sub

    Private Sub BTN_Add_Click(sender As Object, e As EventArgs) Handles BTN_Add.Click

        'Dim command As New MySqlCommand("insert into hefflox.product_table(SuppID,Code,Make,Model,Year,Description,Placement,Brand,GrossPrice,NetPrice,Quantity,Photo) values (@SuppID,@Code,@Make,@Model,@Year,@Description,@Placement,@Brand,@GrossPrice,@NetPrice,@Quantity,@Photo)", CONN)
        ...
        Dim command As MySqlCommand
        Dim lastProdId As Integer = GetLastProdId(1000, 1999) ' Function to retrieve the last ProdID

        ' Determine the category based on the ProdID range
        If lastProdId >= 1000 AndAlso lastProdId < 2000 Then
            ' Category: Axle Hub
            lastProdId += 1 ' Increment the ProdID within the category range
        Else
            ' No previous entries or unknown category
            lastProdId = 1000 ' Start from the beginning for Axle Hub
        End If
    ...
    Private Sub BTN_Edit_Click(sender As Object, e As EventArgs) Handles BTN_Edit.Click
        ...
        Dim update1 As String = "UPDATE hefflox.product_table SET SuppID=@SuppID, Code=@Code, Make=@Make, Model=@Model, Year=@Year, Description=@Description, Placement=@Placement, Brand=@Brand, GrossPrice=@GrossPrice, NetPrice=@NetPrice, Quantity=@Quantity, Photo=@Photo WHERE ProdID=@ProdID"

        Dim command As New MySqlCommand(update1, CONN)
        command.Parameters.AddWithValue("@SuppID", TXTBX_SID.Text)
        command.Parameters.AddWithValue("@Code", TXTBX_Code.Text)
        command.Parameters.AddWithValue("@Make", TXTBX_Make.Text)
        command.Parameters.AddWithValue("@Model", TXTBX_Model.Text)
        command.Parameters.AddWithValue("@Year", TXTBX_Year.Text) ' Assuming Year is a string column
        command.Parameters.AddWithValue("@Description", TXTBX_Description.Text)
        command.Parameters.AddWithValue("@Placement", TXTBX_Placement.Text)
        command.Parameters.AddWithValue("@Brand", TXTBX_Brand.Text)
        command.Parameters.AddWithValue("@GrossPrice", TXTBX_GPrice.Text)
        command.Parameters.AddWithValue("@NetPrice", TXTBX_NPrice.Text)
        command.Parameters.AddWithValue("@Quantity", TXTBX_Q.Text)
        command.Parameters.AddWithValue("@Photo", img)
        command.Parameters.AddWithValue("@ProdID", LBL_ProdID.Text)
    ...
    Private Sub DGV_PT_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles DGV_PT.CellContentClick
