...
Public Class Admin___Brake_Fluid__Categ__v2
    Dim CONN As New MySqlConnection("server = localhost;userid = root;database = hefflox")
    Dim command As MySqlCommand
    Private Sub Bin1()
        Dim command As New MySqlCommand("SELECT * FROM hefflox.product_table WHERE ProdID >= 3000 AND ProdID < 4000 ORDER BY ProdID ASC", CONN)
    ...
    Private Sub Admin___Brake_Fluid__Categ__v2_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Bin1()
    End Sub

    Private Sub BTN_Add_Click(sender As Object, e As EventArgs) Handles BTN_Add.Click
        Dim command As MySqlCommand
        Dim lastProdId As Integer = GetLastProdId(3000, 3999) ' Function to retrieve the last ProdID

        ' Determine the category based on the ProdID range
        If lastProdId >= 3000 AndAlso lastProdId < 4000 Then
            ' Category: Axle Hub
            lastProdId += 1 ' Increment the ProdID within the category range
        Else
            ' No previous entries or unknown category
            lastProdId = 3000 ' Start from the beginning for Axle Hub
        End If
    ...
    Private Sub Admin___Brake_Fluid__Categ__v2_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles DGV_PT.CellContentClick
